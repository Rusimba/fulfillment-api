...
async createOrder(userId: number, dto: CreateOrderDto) {
  return this.prisma.$transaction(async (tx) => {
    ...
    // 3. Создание заказа + списание остатков + фиксация цены
...
getProfile(@Request() req) {
  return req.user; // ← Guard уже расшифровал токен
}
...
model OrderItem {
  id        Int     @id @default(autoincrement())
  quantity  Int
  price     Float   // ⚡ Цена на момент покупки (важно!)
  orderId   Int
  order     Order   @relation(fields: [orderId], references: [id])
  productId Int
  product   Product @relation(fields: [productId], references: [id])
}
Ключевое решение: OrderItem.price — копия цены товара на момент покупки. Если позже цена товара изменится, исторические заказы останутся с корректной суммой.
🐳 Docker
...
docker-compose.yml поднимает:
    ...
    PostgreSQL 16 с healthcheck (API не запустится, пока БД не готова)
    NestJS API с правильным порядком запуска через depends_on
...
🗺 Roadmap
    ...
    RabbitMQ для event-driven архитектуры (email/SMS уведомления)
